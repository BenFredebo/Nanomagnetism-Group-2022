...
class Keithley220():
    ...
    def make_ramp_wave(self, I_max, period, points=100):
        '''
        ...
        ----------
        I_max : float
            upper bound on desired wave amplitude
        period : float
            length of one iteration of wave, dwell time is then period/points
    ...
    def reset_all_memory(self):
        '''
        Resets all memory locations for the Keithley
        ...
        for i in range(1, 101):
            self.set_current(0, i)
            self.set_dwell_time(3e-3, i)
            self.set_vlimit(1, i)

        self.kill()
        self.terminate_current()
        self.set_program_mode('step')
